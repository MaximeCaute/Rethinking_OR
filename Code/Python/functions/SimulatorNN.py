...
class Client:
    ...
    def get_representation_nn(self):
        current_n = self.G.nodes[self.ar_node]
        dest_n = self.G.nodes[self.dest_node]
        return [current_n['x'], current_n['y'], dest_n['x'], dest_n['y']]
...
class Vehicle:
    ...
    def __str__(self):
        return 'Vehicle {} with destination {} and {} clients and {} future clients'.format(self.ID, self.destination,
                                                                                            len(self.clients),
                                                                                            len(self.clients_to_pick))
    ...
    def next_destination(self):
        if not self.clients and not self.clients_to_pick:
            ...
            for client in self.clients:
                ...
                distance_client = nx.shortest_path_length(self.G, self.mapping[self.current_node],
                                                          self.mapping[self.clients[client]], 'distance')
            ...
            if len(self.clients) < 5:
                for client in self.clients_to_pick:
                    # compute distance to client destination
                    distance_client = nx.shortest_path_length(self.G, self.mapping[self.current_node],
                                                              self.mapping[self.clients_to_pick[client]], 'distance')
                    if distance_client <= distance:
                        distance = distance_client
        ...
        plan_osmnx = nx.shortest_path(self.G, self.mapping[self.current_node], self.mapping[self.destination],
                                      'distance')
        ...
        for i in range(len(self.plan) - 1):
            t = nx.shortest_path_length(self.G, self.mapping[self.plan[i]], self.mapping[self.plan[i + 1]],
                                        weight='time')
    ...
    def drop_client(self, client):
        ...
        client.dropoff_t = self.time
        self.clients_carried.append(client)
        # update plan
        self.next_destination()
    ...
    def update(self, t):
        ...
        if len(self.plan) == 0:
            ...
            while self.time + self.plan_time[-1] < t:
                ...
                self.ride_time += self.plan_time[-1]
                self.movements.append(self.current_node)
                # dropoff clients
            ...
            if len(time_to_cities) > 0:
                ...
                self.ride_time += time_to_cities[-1]
    ...
    def get_representation_nn(self):
        ...
        # load, queue, current location, future location
        return [len(self.clients), future_c, x_current, y_current, x_next, y_next]
...
class Simulation:
    ...
    def get_next_customer_nn(self):
        """
        updates current customer and time information and provides representation for NN
        """
        if self.current_customer >= len(self.customers) - 1:
            return -1, -1
        else:
            ...
            self.current_customer += 1
            client = self.customers[self.current_customer]
            representation.append(client.get_representation_nn())
            self.time = client.arrival

            # update vehicles position
            for v in self.vehicles:
                v.update(self.time)
                representation.append(v.get_representation_nn())
...
class Scenario:
    ...
    def __init__(self, table_number, NN=False):
        # instantiate simulation
        Event = Simulation(table_number)
        model = torch.load('model_try1.pt')
        client = 0
        while client != -1:
            # get next customer
            if not NN:
                ...
                if client != -1:
                    next_vehicle = get_vehicle_assignment_nn(client, state, 0)
